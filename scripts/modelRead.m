...
function modeldat = modelRead(scenario, range, M)

if nargin ~= 2 && nargin ~= 3
    error('Invalid input to modelRead.m. Revise plz.'); 
end

%ifile = ['data\door_12in_v2\i_data_' scenario '_' num2str(range) '.txt'];
%qfile = ['data\door_12in_v2\q_data_' scenario '_' num2str(range) '.txt'];
